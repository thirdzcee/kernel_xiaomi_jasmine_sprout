...
static void change_elevator(struct req_queue_data *r, bool use_row)
{
	struct request_queue *q = r->queue;

	if (r->using_row == use_row)
		return;
	...
	if (use_row) {
		...
		elevator_change(q, ROW_IOSCHED);
	} else {
		elevator_change(q, r->prev_e);
	}
}

static void change_all_elevators(struct list_head *head, bool use_row)
{
	struct req_queue_data *r;

	list_for_each_entry(r, head, list)
		change_elevator(r, use_row);
}
		...
		unsigned long action, void *data)
{
	...
	case FB_BLANK_UNBLANK:
		/*
		 * Switch back from row to the original iosched after a delay
	...
	default:
		/*
		 * Switch to row when the screen turns off. Purposely block
