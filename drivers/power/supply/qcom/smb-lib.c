				...
				void *data, int disable, const char *client)
{
	...
	#if (defined (CONFIG_MACH_XIAOMI_JASMINE) || defined (CONFIG_KERNEL_CUSTOM_E7S) || defined(CONFIG_KERNEL_CUSTOM_F7A) || defined(CONFIG_KERNEL_CUSTOM_E7T))
	disable = 0;
	#endif
	if (disable) {
		/*
...
#if defined(CONFIG_MACH_XIAOMI_JASMINE)
...
#elif defined(CONFIG_MACH_XIAOMI_JASMINE)
...
static void smblib_force_legacy_icl(struct smb_charger *chg, int pst)
{
	...
	case POWER_SUPPLY_TYPE_USB_FLOAT:
		...
		#if defined (CONFIG_KERNEL_CUSTOM_E7S)
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 500000);
		#elif defined(CONFIG_MACH_XIAOMI_JASMINE) || defined(CONFIG_KERNEL_CUSTOM_F7A)
		vote(chg->usb_icl_votable, USER_VOTER, false, 0);
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	if (chg->float_rerun_apsd) {
		...
		if (apsd_result->bit & QC_2P0_BIT) {
			...
			#if defined (CONFIG_MACH_XIAOMI_JASMINE) || defined(CONFIG_KERNEL_CUSTOM_F7A) || defined(CONFIG_KERNEL_CUSTOM_E7T)
			vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 1000000);
			#else
			vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 500000);
			#endif
